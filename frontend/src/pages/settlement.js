    ...
    function renderTeeSelector() {
      ...
      maleTees.forEach(tee => {
        ...
        const btn = el("button", {
          ...
          style: (() => {
            const lightColors = ["white","#fff","#ffffff","yellow","#ffff00","silver","#c0c0c0","cream","beige"];
            const isLight = lightColors.some(lc => (tee.tee_color || "").toLowerCase().includes(lc));
            const textColor = isSelected ? "" : (isLight ? "#1a1a1a" : "#fff");
            const textShadow = isSelected ? "" : (isLight ? "none" : "0 1px 2px rgba(0,0,0,0.4)");
            const bg = isSelected ? "" : (tee.tee_color || "transparent");
            const border = tee.tee_color || "var(--green)";
            return `background:${bg};border-color:${border};color:${textColor};text-shadow:${textShadow}`;
          })()
